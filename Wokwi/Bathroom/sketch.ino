...
void connect() {
  ...
  Serial.print("\nconnecting...");
  while (!client.connect("Bathroom","mrnyahya","QTzQsqlMb9m8gB9i")) {
    Serial.print(".");
    delay(1000);
  }
  ...
  client.subscribe("mrnyahya/bathroom/led");
...
void loop() {
 ...
 if ((millis() - timerDHT) > WAKTUDHT) {
    ...
    client.publish("mrnyahya/bathroom/suhu", msg, true,1); //publish message suhu
    snprintf (msg, MSG_DHT, "%s", itoa(kelembapan,msg,10)); //ubah nilai int ke string
    Serial.print("Kelembapan : ");
    Serial.println(kelembapan);
    client.publish("mrnyahya/bathroom/kelembapan", msg, true,1); //publish message kelembapan
